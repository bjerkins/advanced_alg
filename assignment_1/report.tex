...
The vertices in the flow graph will be the faces and the original vertices of the planar embedding. There will be edges between adjacent faces and between vertices which lie on the boundary cycle of a face.

Demands for faces are given by the formula
\begin{align*}
    demand(f) = \begin{cases} 4 & \text {if f is internal}\\ -4 &\text{otherwise}\end{cases}
\end{align*}

The demand for an original vertex $v$ is given by
\begin{align*}
